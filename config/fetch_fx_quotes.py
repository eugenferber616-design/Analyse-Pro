...
def main(wl, outcsv):
    ...
    for s in syms:
        rl.wait()
        try:
            r = requests.get(f"{API}/forex/quote", params={"symbol": s, "token": TOKEN}, timeout=30)
            if r.status_code == 429: time.sleep(2); r = requests.get(f"{API}/forex/quote", params={"symbol": s, "token": TOKEN}, timeout=30)
            r.raise_for_status()
            j = r.json() or {}
            rows.append({"pair": s, "bid": j.get("bid"), "ask": j.get("ask"), "last": j.get("price"), "timestamp": j.get("timestamp")})

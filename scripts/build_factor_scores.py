...
# -*- coding: utf-8 -*-
"""
build_factor_scores.py – Factor Scores für FactorScoreScanner-Indikator

Erzeugt: data/processed/factor_scores.csv

- Global-Score  (Quality + Growth)
- Value-Score   (adaptiv: Large Caps vs. Russell/SmallCap)
- Momentum-Score
- Risk-Score
- R2K-Growth-Grade (für Small Caps / Russell)
- Style-Bucket: LARGE / R2K / OTHER
- Value-Mode:   CLASSIC / R2K
"""
...
def scale_linear(x: pd.Series, low_bad: float, high_good: float) -> pd.Series:
    """0–100: low_bad -> 0, high_good -> 100, linear dazwischen."""
    v = to_num(x)
...
def scale_inverse(x: pd.Series, low_good: float, high_bad: float) -> pd.Series:
    """0–100: low_good -> 100, high_bad -> 0, linear dazwischen."""
    v = to_num(x)
...
def as_percent(x: pd.Series) -> pd.Series:
    """Wenn Median zwischen -1 und 1 liegt → als Prozent interpretieren und *100."""
    v = to_num(x)
...
# ------------------------------------------------------------------ #
# Price-Features (mit Unterordner pro Anfangsbuchstaben)             #
# ------------------------------------------------------------------ #

def compute_price_features(sym: str) -> Dict[str, float]:
    """
    Liest data/prices/{SUBDIR}/{SYM}.csv und berechnet:
      - rtn_1m / 3m / 6m / 12m (in %)
      - near_52w_high (0–100: Nähe zum 52W Hoch)
    """
    if not sym:
        return {}
...
def main():
    ...
    if (cds is None or cds.empty) and os.path.exists(os.path.join(BASE_PROC, "cds_proxy.csv.gz")):
    ...
    if (si is None or si.empty) and os.path.exists(os.path.join(BASE_PROC, "short_interest.csv.gz")):
    ...
    # 3) PRICE-FEATURES (Momentum & 52W-High)
    ...
    # ------------------------------------------------------------------
    # 4) STYLE-BUCKETS: LARGE vs. R2K vs. OTHER
    # ------------------------------------------------------------------
    idx_df = rd("index_membership.csv")
    mask_r2k = pd.Series(False, index=fund.index)
    mask_large = pd.Series(False, index=fund.index)
    ...
    if idx_df is not None and not idx_df.empty and "symbol" in idx_df.columns and "index" in idx_df.columns:
        ...
        # Large Caps: SP500 / SPX / NASDAQ / NDX / DOW / DJI
        large_mask_idx = (
            idx_df["index"].str.contains("SP5") |
            idx_df["index"].str.contains("S&P") |
            idx_df["index"].str.contains("SPX") |
            idx_df["index"].str.contains("NASDAQ") |
            idx_df["index"].str.contains("NDX") |
            idx_df["index"].str.contains("DOW") |
            idx_df["index"].str.contains("DJI")
        )
        large_syms = idx_df.loc[large_mask_idx, "symbol"].unique()
        mask_large = fund["symbol"].isin(large_syms)
    else:
        # Fallback: nach Marketcap splitten
        mc = to_num(fund["marketcap"])
        if mc.notna().sum() > 10:
            # Untere 40% → R2K/SmallCap
            q40 = mc.quantile(0.40)
            # Obere 30% → Large
            q70 = mc.quantile(0.70)
            mask_r2k = mc <= q40
            mask_large = mc >= q70

    fund["style_bucket"] = "OTHER"
    fund.loc[mask_large, "style_bucket"] = "LARGE"
    fund.loc[mask_r2k, "style_bucket"]   = "R2K"

    # ------------------------------------------------------------------
    # 5) VALUE-SCORES (CLASSIC vs. R2K-EARLY-VALUE)
    # ------------------------------------------------------------------
    ...
    value_score_large = np.nanmean(value_mat_large, axis=0)

    # Early-Value für R2K/SmallCaps (nicht-gewinnende Firmen, aber gute Story)
    for c in ["ps", "ev_sales", "fcf_yield", "gross_margin", "oper_margin", "rev_yoy"]:
        if c not in fund.columns:
            fund[c] = np.nan

    sc_ps_r2k       = scale_inverse(fund["ps"],        low_good=1.0,  high_bad=15.0)
    sc_ev_sales_r2k = scale_inverse(fund["ev_sales"],  low_good=1.0,  high_bad=12.0)
    sc_gm_r2k       = scale_linear(as_percent(fund["gross_margin"]), low_bad=0.0,  high_good=50.0)
    sc_om_r2k       = scale_linear(as_percent(fund["oper_margin"]),  low_bad=-20.0, high_good=20.0)
    sc_rev_yoy_r2k  = scale_linear(as_percent(fund["rev_yoy"]),       low_bad=-10.0, high_good=30.0)
    sc_fcf_y_r2k    = scale_linear(fund["fcf_yield"],                 low_bad=-5.0,  high_good=10.0)

    value_mat_r2k = np.vstack([
        sc_ps_r2k,
        sc_ev_sales_r2k,
        sc_gm_r2k,
        sc_om_r2k,
    ...
    value_score_r2k_raw = np.nanmean(value_mat_r2k, axis=0)

    # Adaptive Zuweisung
    fund["value_score_large"] = value_score_large
    fund["value_score_r2k"]   = np.nan
    fund.loc[mask_r2k, "value_score_r2k"] = value_score_r2k_raw[mask_r2k.values]

    # Default: Classic
    fund["value_score"] = fund["value_score_large"]
    fund["value_mode"]  = "CLASSIC"
    # Für R2K: Early-Value
    fund.loc[mask_r2k, "value_score"] = fund.loc[mask_r2k, "value_score_r2k"]
    fund.loc[mask_r2k, "value_mode"]  = "R2K"
    ...
    for cname in fund.columns:
        lc = cname.lower()
        if "rev_yoy" in lc or "revenue_yoy" in lc or \
           "rev_cagr" in lc or "revenue_cagr" in lc or \
           "eps_yoy" in lc:
    ...
    fund["momentum_score"] = np.nanmean(
        np.vstack([sc_m_1m, sc_m_3m, sc_m_6m, sc_m_12m, sc_m_hi]),
        axis=0
    )
    ...
    # ------------------------------------------------------------------
    # 10) GLOBAL/FUNDAMENTAL + GRADES (für deinen Indikator)
    # ------------------------------------------------------------------
    fund["global_score"] = 0.50 * fund["quality_score"] + 0.50 * fund["growth_score"]
    fund["fundamental_score"] = fund["global_score"]
    fund["composite_score"]   = fund["global_score"]

    # Grades
    fund["global_grade"]   = score_to_grade(fund["global_score"])
    ...
    # R2K Growth Grade (nur für Small Caps / Russell)
    fund["r2k_growth_grade"] = np.nan
    fund.loc[mask_r2k, "r2k_growth_grade"] = score_to_grade(fund.loc[mask_r2k, "growth_score"])

    # Optional: separater R2K-Growth-Score, falls du ihn mal brauchst
    fund["r2k_growth_score"] = np.nan
    fund.loc[mask_r2k, "r2k_growth_score"] = fund.loc[mask_r2k, "growth_score"]

    # ------------------------------------------------------------------
    # 11) OUTPUT – genau das, was FactorScoreScanner erwartet + Extras
    # ------------------------------------------------------------------
    out_cols = [
        "symbol", "sector", "industry", "marketcap",
        "style_bucket", "value_mode",

        # Basis-Metriken (Debug / Analyse)
        "pe", "pb", "ps", "ev_ebitda", "ev_sales",
        "gross_margin", "oper_margin", "fcf_margin",
        "beta", "hv20", "hv60", "net_debt_ebitda",
        "current_ratio", "credit_spread",
        "si_pct_float", "borrow_rate",
        "rtn_1m", "rtn_3m", "rtn_6m", "rtn_12m", "near_52w_high",

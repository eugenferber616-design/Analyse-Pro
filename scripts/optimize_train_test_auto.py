...
def _to_dt_index(df: pd.DataFrame, date_col: str = "date") -> pd.DataFrame:
    """erzwingt DatetimeIndex; nimmt notfalls die erste Spalte als Datum"""
    if df.empty:
        return df
...
def _hyst(x: pd.Series, on: float, off: float,
    ...
    """Hysterese: <= on → long; >= off → flat/short (je nach mode)."""
    pos, last = [], 0.0
    for v in x.values:
        if np.isnan(v):
...
# Gemeinsame tägliche Achse robust (kein min()/max() über gemischte Typen!)
...
for i in range(0, len(years) - 6):
    ...
    # Grid auf Train → bestes Set wählen
        ...
        for on, off in thresholds:
            for mode in modes:
                ...
                for sw in sw_list:
                    ...
                    key = (sh, cg, -abs(dd))  # Sharpe → CAGR → kleinerer Drawdown
                    if (best_key is None) or (key > best_key):
                        best_key, best_params = key, dict(
                            ema_span=span, on=on, off=off, mode=mode, short_w=sw
                        )

    sh_te, cg_te, dd_te = eval_params(
        best_params["ema_span"], best_params["on"], best_params["off"],
        best_params["mode"], best_params["short_w"], ret[te], sc[te]
    )
